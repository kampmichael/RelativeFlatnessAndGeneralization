...
BS = 64
EPOCHS = 250
feature_layer_id = 62 # penultimate layer
lmb = 0.1 # regularization coefficient
...
class LitRes18Net(pl.LightningModule):
    ...
    def loss(self, xs, ys):
        ...
        if self.training: # compute FAM regularized loss
            ...
            loss = criterion(outputs, ys) + lmb * FAMreg(xs, ys, layer_hessian,
                                                         norm_function='neuronwise', approximate=True)
        else:
